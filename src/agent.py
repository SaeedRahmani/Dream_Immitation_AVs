...
class Agent:
    def __init__(self, policy_model, env=None):
        """
        Args:
            policy_model : Pretrained policy model (A2C, PPO, etc.)
            env : Gym or custom environment
        """
        self.policy_model = policy_model
        self.env = policy_model.get_env() if env is None else env
        self.state = self.env.reset()

    @classmethod
    def from_sb3_file(cls, policy_model_path, algo_name, env, **kwargs):
        """
        Load a pretrained policy model using Stable-Baselines3.
        """
        policy_model = algo_name.load(policy_model_path, env=env, **kwargs)
        return cls(policy_model)

    def predict(self, state, deterministic=False):
        """
        Predict the next action based on the current state.
        """
        action, _ = self.policy_model.predict(state, deterministic=deterministic)
        return action
